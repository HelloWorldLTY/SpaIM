...
def val(opt):
    ...
    with torch.no_grad():
        eval_result = None
        input_result = None
        for i, (seq, st_style, spa) in enumerate(valdataloader):
            inputs = {
                'scx': seq,
                'st_style': st_style,
                # 'sc_cls': seq_cls
        ...
        # impute_result = out['sc_fake'].detach().cpu().numpy()
    ...
    # eval_result = np.expm1(eval_result)
    # input_result = np.expm1(input_result)
...
def train(opt):
    ...
    # for epoch in range(opt.epochs):
        for i, (seq, spa, sc_style, st_style) in enumerate(dataloader):
            inputs = {
                'scx': seq,
                'stx': spa,
                'sc_style': sc_style,

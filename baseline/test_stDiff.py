...
parser.add_argument("--document", type=str, default='Dataset1')
...
# Dataset24_nano9-1 \ Dataset22_nano5-3 \ Dataset32
adata_spatial = sc.read_h5ad('/data/boom/SpaIM/dataset/Dataset22/' + args.sp_data)
adata_seq = sc.read_h5ad('/data/boom/SpaIM/dataset/Dataset22/'+ args.sc_data)
...
# 提取表达矩阵并转换为 DataFrame
df_spatial = adata_spatial.to_df()
df_seq = adata_seq.to_df()

# 转换列名为小写
df_spatial.columns = df_spatial.columns.str.lower()
df_seq.columns = df_seq.columns.str.lower()

# 提取公共列
common_columns = df_spatial.columns.intersection(df_seq.columns)

# 选择公共列
df_spatial = df_spatial[common_columns]
df_seq = df_seq[common_columns]

# 将 DataFrame 转回 AnnData 格式
adata_spatial = AnnData(df_spatial)
adata_seq = AnnData(df_seq)
...
class CalculateMeteics:
    ...
    def compute_all(self):
        ...
        prefix = self.prefix
        print('Shape:',raw.shape, impute.shape) # SSIM: (645, 915) (1, 8)

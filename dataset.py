...
class ImputationNano(Dataset):
    def __init__(self, opt, istrain='train'):
        self.opt = opt
        seq_data = self.load_data(opt.seq_data)
        self.spa_data = self.load_data(opt.spa_data)
        if 'leiden' in opt.cluster:
            self.seq_cluster = seq_data.obs[opt.cluster].cat.codes.values
        elif opt.cluster == 'annotate':
            self.seq_cluster = seq_data.obs['merge_cell_type'].cat.codes.values
        else:
            self.seq_cluster = [1]

        self.style_dim = opt.style_dim
        self.istrain = istrain
        self.seq_data = self.aggreate_cell_types(seq_data)
    ...
    def aggreate_cell_types(self, adata):
        if 'leiden' in self.opt.cluster or self.opt.cluster == 'annotate':
            x = adata.X
    ...
    def __getitem__(self, index):
        ...
        if self.istrain == 'train':
            ...
            return torch.FloatTensor(seq_x), st_style, torch.FloatTensor(spa_x)
        elif self.istrain == 'debug_train':
            seq_x = self.seq_train.X[index, ...]
            spa_x = self.spa_train.X[index, ...]
            return torch.FloatTensor(seq_x), torch.FloatTensor(spa_x), sc_style, st_style
...
class ImputationDataset(Dataset):
    def __init__(self, opt, istrain='train'):
        self.opt = opt
        self.seq_data = self.load_data(opt.seq_data)
        self.spa_data = self.load_data(opt.spa_data)

        self.seq_cluster = self.seq_data.obs['leiden'].cat.codes.values
        self.style_dim = opt.style_dim
        self.istrain = istrain
        self.seq_data = self.aggreate_cell_types()
        ...
        test_gene = set(test_gene) & set(self.seq_data.var_names)
        ...
        self.seq_train = self.seq_data[:, train_gene].copy().T
        self.seq_val = self.seq_data[:, val_gene].copy().T
        self.seq_test = self.seq_data[:, test_gene].copy().T
